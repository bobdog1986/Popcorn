namespace Popcorn.FFME.Decoding
{
    ...
    /// </summary>
    /// <seealso cref="Popcorn.FFME.Decoding.MediaComponent" />
        ...
        private void InitializeFilterGraph(AVFrame* frame)
        {
            ...
            try
            {
                ...
                fixed (AVFilterContext** sink = &SinkFilter)
                {
                    ...
                    if (result != 0)
                        throw new MediaContainerException($"{nameof(ffmpeg.avfilter_graph_create_filter)} (buffersink) failed. Error {result}: {FFmpegEx.GetErrorMessage(result)}");
